...
test("slash menu selection index ignores the welcome header help hint", () => {
  const header = `${SELECTED_COMPLETION_SGR}omfx\x1b[0m${DIM_SGR} v0.3.27 · Run /help for commands`;
...
describe.skipIf(SKIP)("tui: slash menu", () => {
  ...
  test(
    ...
    async () => {
      ...
      await session.sendKeys("Escape");
      await session.waitForPane(
        (current) => hasEmptyComposer(current) && current.includes("omfx") && !current.includes("Commands"),
        5_000,
      );
      ...
      await session.sendKeys("Escape");
      await session.waitForPane(
        (current) => hasEmptyComposer(current) && current.includes("omfx") && !current.includes("Sessions"),
        5_000,
      );
  ...
  test(
    ...
    async () => {
      ...
      let pane = grid.join("\n");
      expect(pane).not.toContain("omfx");
      ...
      await session.sendKeys("Escape");
      await session.waitForPane(
        (current) => hasEmptyComposer(current) && current.includes("omfx") && !current.includes("Enter Open"),
        5_000,
      );
  ...
  test(
    ...
    async () => {
      ...
      expect(tail_history).not.toContain("omfx v0.3.7");
  ...
  test(
    ...
    async () => {
      ...
      await session.sendKeys("C-[");
      await session.waitForPane(
        (current) => hasEmptyComposer(current) && current.includes("omfx") && !current.includes("Tab Provider"),
        5_000,
      );
