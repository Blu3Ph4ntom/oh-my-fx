...
/// Identifies omfx on every AI Gateway request; the zig std.http default
/// (`zig/<version> (std.http)`) is never sent to the gateway.
pub const user_agent = product_identity.name ++ "/" ++ build_options.app_version;
...
fn gatewayExtraHeaders(
    ...
    len += 1;
    buf[len] = .{ .name = "X-Title", .value = "omfx" };
    len += 1;
...
test "gateway chat request sends omfx user agent and attribution headers" {
    ...
    try std.testing.expectEqualStrings("omfx", fixture.capturedHeaderValue("x-title").?);
