...
fn ensureManagedDir(parent_path: []const u8, child_name: []const u8) !void {
    ...
    for (0..2) |_| {
        ...
        io_mod.setPermissions(child, io_mod.permissionsFromMode(0o700)) catch
...
test "web_fetch artifact store creates private managed directories" {
    ...
    try std.testing.expectEqual(io_mod.expectedTestMode(0o700), io_mod.permissionsToMode(artifacts_stat.permissions) & 0o777);
    try std.testing.expectEqual(io_mod.expectedTestMode(0o700), io_mod.permissionsToMode(web_fetch_stat.permissions) & 0o777);
}
