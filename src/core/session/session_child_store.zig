...
pub const SessionChildCapability = struct {
    ...
    pub fn initLegacyRoute(
        ...
        if (mode == .writable) {
            io_mod.setPermissions(route, private_dir_permissions) catch
                return error.PrivateStatePermissionsUnsupported;
        }
    ...
    pub fn initLegacyBackgroundRoutes(
        ...
        if (mode == .writable) {
            io_mod.setPermissions(records, private_dir_permissions) catch return error.PrivateStatePermissionsUnsupported;
        }
        ...
        if (logs) |route| {
            if (mode == .writable) {
                io_mod.setPermissions(route, private_dir_permissions) catch return error.PrivateStatePermissionsUnsupported;
            }
            try verifyPrivateDirectory(route);
        }
...
test "terminal capabilities are private route restricted and reject symlinks" {
    ...
    try std.testing.expectEqual(io_mod.expectedTestMode(0o700), io_mod.permissionsToMode(terminal_stat.permissions) & 0o777);
    try std.testing.expectEqual(io_mod.expectedTestMode(0o700), io_mod.permissionsToMode(state_stat.permissions) & 0o777);
    try std.testing.expectEqual(io_mod.expectedTestMode(0o600), io_mod.permissionsToMode(record_stat.permissions) & 0o777);
