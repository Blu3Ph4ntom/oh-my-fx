...
pub const Store = struct {
    ...
    fn ensureWritable(self: *Store) !void {
        if (self.durable_home == null) {
            ...
            defer home.close();
            self.durable_home = profile_paths.openOrCreateRootDir(
                &home,
    ...
    fn openUsage(
        ...
        if (!io_mod.permissionsIsPrivateFile(verified.permissions)) {
            return error.PrivateStatePermissionsUnsupported;
        }
        if (created) {
...
test "profile usage store repairs an existing profile directory to private mode" {
    ...
    try std.testing.expectEqual(io_mod.expectedTestMode(0o700), io_mod.permissionsToMode(stat.permissions) & 0o777);
}
...
test "profile usage reads reject an unsafe profile directory without repairing it" {
    ...
    const stat = try profile.stat(io_mod.getIo());
    try std.testing.expectEqual(io_mod.expectedTestMode(0o755), io_mod.permissionsToMode(stat.permissions) & 0o777);
}
