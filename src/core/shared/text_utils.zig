...
pub fn sanitizeAssistantText(text: []const u8) []const u8 {
    ...
    const is_intro = containsIgnoreCase(first_line, "i'm omfx") or
        containsIgnoreCase(first_line, "i am omfx") or
...
test "sanitizeAssistantText strips first-line assistant intro" {
    const cleaned = sanitizeAssistantText(" \tI'm omfx, your local coding assistant.\n\n  Here is the answer.\n");
    try std.testing.expectEqualStrings("Here is the answer.", cleaned);

    const unchanged = sanitizeAssistantText("I'm omfx, your local coding assistant.");
    try std.testing.expectEqualStrings("I'm omfx, your local coding assistant.", unchanged);
}
