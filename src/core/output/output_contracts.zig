...
pub const SessionListSnapshot = struct {
    ...
    pub fn renderText(self: SessionListSnapshot, alloc: Allocator) ![]u8 {
        ...
        if (self.has_more) {
            try out.writer.print(
                "[sessions] more saved sessions; continue with `omfx sessions {s}--cursor {s}`\n",
                .{ if (self.all_workspaces) "--all " else "", self.next_cursor orelse "" },
            );
        }
        if (self.skipped_invalid > 0) {
            try out.writer.print(
                "[sessions] warning: skipped {d} unreadable saved session{s}; run `omfx doctor` for recovery guidance\n",
...
pub const UpgradeSnapshot = struct {
    ...
    pub fn renderText(self: UpgradeSnapshot, alloc: Allocator) ![]u8 {
        ...
        switch (self.status) {
            ...
            .up_to_date => {
                if (std.mem.eql(u8, self.channel, "dev") and self.latest_revision.len > 0) {
                    try out.writer.print("omfx dev {s} is already up to date (", .{shortRevision(self.latest_revision)});
                } else {
                    try out.writer.writeAll("omfx is already up to date (");
                }
                try writeVersionWithPrefix(&out.writer, self.latest);
                try out.writer.writeAll(")\n");
            },
...
test "core status snapshot text and json stay stable" {
    const snapshot = StatusSnapshot{
        .model = "alpha",
        .auth_help = "omfx needs access to Vercel AI Gateway. Run omfx login to sign in, omfx setup to use an API key, or set AI_GATEWAY_API_KEY.",
        .permission_mode = .ask,
...
test "core status snapshot includes selected team when present" {
    ...
    defer std.testing.allocator.free(json);
    try std.testing.expectEqualStrings(
        "{\"kind\":\"status\",\"model\":\"alpha\",\"update_channel\":\"stable\",\"build_channel\":\"stable\",\"build_revision\":\"\",\"auth\":\"omfx login\",\"auth_refreshable\":true,\"team\":\"example-team\",\"permission_mode\":\"ask\",\"sandbox\":\"none\",\"workspace\":\"/tmp/fx\",\"history_turns\":0,\"session_permission_grants\":0,\"agent_step_limit\":24}",
        json,
    );
}
...
test "core session list snapshot text and json stay stable" {
    ...
    try std.testing.expectEqualStrings(
        "[sessions] 1 saved\n - Session title\n   id=abc | 3 turns | Spanish | updated 1970-01-01 00:00:00.002 UTC\n" ++
            "[sessions] more saved sessions; continue with `omfx sessions --cursor v1:2:abc`\n",
        paged_text,
    );
...
test "core upgrade snapshot renders errors and statuses" {
    ...
    try std.testing.expectEqualStrings("omfx is already up to date (v0.2.10)\n", up_to_date_text);
