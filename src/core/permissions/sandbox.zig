...
pub const unsupported_os_sandbox_message = "operating system sandbox is not available on this host because omfx has no OS sandbox implementation here. Choose `none` to run commands without sandbox isolation.";
...
fn currentProcessId() u64 {
    // `std.c.getpid` is a process HANDLE on Windows, not a numeric pid.
    if (comptime builtin.os.tag == .windows) return @intCast(@intFromPtr(std.c.getpid()));
