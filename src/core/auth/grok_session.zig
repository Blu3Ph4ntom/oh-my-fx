...
fn loadFromDir(alloc: Allocator, fx_dir: *std.Io.Dir, report_open_failure: bool) !?Session {
    ...
    if (stat.kind != .file or io_mod.permissionsOpenToGroupOther(stat.permissions)) {
...
fn openExistingPrivateFxDir(home_dir: *io_mod.VerifiedDir) !io_mod.VerifiedDir {
    ...
    if (!io_mod.permissionsOwnerWritable(initial_stat.permissions)) return error.PrivateStatePermissionsUnsupported;
