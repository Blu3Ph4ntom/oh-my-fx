    ...
    for (servers) |*server| {
        ...
        switch (mode) {
            .oauth => try out.writer.writeAll(
                ",\"interactive\":true,\"message\":\"Run /mcp auth for this server in an interactive omfx session.\"",
            ),
            .bearer_environment => {
                try out.writer.writeAll(
                ...
                try out.writer.writeAll(
                    ",\"message\":\"Set this environment variable before starting omfx.\"",
                );
            },
        }
        try out.writer.writeAll("}}");
...
fn writeModernRequestMetadataWithProgress(
    ...
    try writer.writeAll("\",\"io.modelcontextprotocol/clientInfo\":{\"name\":\"omfx\",\"version\":");
...
fn buildLegacyInitializeRequest(
    ...
    try out.writer.writeAll("},\"clientInfo\":{\"name\":\"omfx\",\"version\":");
...
test "modern request builders share required request metadata" {
    ...
    const metadata = try std.fmt.allocPrint(
        alloc,
        "\"_meta\":{{\"io.modelcontextprotocol/protocolVersion\":\"{s}\",\"io.modelcontextprotocol/clientInfo\":{{\"name\":\"omfx\",\"version\":\"{s}\"}},\"io.modelcontextprotocol/clientCapabilities\":{{}}}}",
