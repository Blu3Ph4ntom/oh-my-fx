...
fn answerUrl(
    ...
    const failure_question = try std.fmt.allocPrint(
        alloc,
        "omfx could not open the browser for {s}. The URL was not fetched. Continue manually, retry the browser, or cancel?",
        .{display_host},
    );
...
fn answerLegacyUrlCompletion(
    ...
    const question = try std.fmt.allocPrint(
        alloc,
        "Complete the browser flow requested by MCP server {s}. omfx will continue automatically if the server confirms every URL request. Otherwise choose I completed it / Retry, or Cancel.",
        .{display_server_name},
    );
    defer alloc.free(question);
