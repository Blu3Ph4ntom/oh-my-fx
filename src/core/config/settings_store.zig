...
pub const Store = struct {
    ...
    pub fn initFromHome(alloc: Allocator, home_path: []const u8, mode: OpenMode) !Store {
        ...
        const durable_mode: u32 = io_mod.permissionsToMode(stat.permissions);
        if (mode == .writable and !io_mod.permissionsIsPrivateDir(stat.permissions)) {
            return error.PrivateStatePermissionsUnsupported;
        }
    ...
    fn loadRawPrimary(self: *Store, alloc: Allocator) !RawPrimary {
        ...
        const primary_mode: u32 = io_mod.permissionsToMode(verified_stat.permissions);
        if (self.mode == .writable and !io_mod.permissionsIsPrivateFile(verified_stat.permissions)) {
            return error.PrivateStatePermissionsUnsupported;
        }
...
test "user patch snapshots and removes legacy workspace copies" {
    ...
    try std.testing.expectEqual(io_mod.expectedTestMode(0o600), io_mod.permissionsToMode(recovery_stat.permissions) & 0o777);
...
test "missing user settings is created through private durable commit" {
    ...
    try std.testing.expectEqual(io_mod.expectedTestMode(0o600), io_mod.permissionsToMode(stat.permissions) & 0o777);
}
