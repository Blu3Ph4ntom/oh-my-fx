    ...
    pub fn userSettingsPatch(self: SessionPreferencePatch) config_runtime.UserSettingsPatch {
        ...
        if (self.provider) |provider| {
            switch (provider) {
                ...
                .openai, .openrouter, .xai, .deepseek, .groq, .cerebras, .fireworks, .together, .mistral, .google, .openai_compatible => patch.model = self.model,
...
test "upgrade resume restores active session with the installed version notice" {
    ...
    try std.testing.expectEqualStrings("● omfx has been updated to v9.9.9", app.notices.items[0]);
