...
const memory_description =
    "Save, list, or clear durable user preferences for future omfx sessions. When to use: the user explicitly asks to remember, forget, save, or recall a preference. When NOT to use: store task notes, secrets, project facts, temporary context, or anything the user did not ask to persist.";
...
const install_skill_description =
    "Install a reusable skill from a supported source into omfx managed skill storage. When to use: the user asks to install a skill or pastes a skills install command. When NOT to use: no installation is required, install packages, fetch unrelated repos, or modify project code.";
...
test "terminal dispatch is permission gated and fails closed when unavailable" {
    ...
    try std.testing.expectEqualStrings(
        "Durable terminal actions require a saved omfx session.",
...
test "built-in subagent owns product metadata schema and callbacks" {
    ...
    try std.testing.expect(std.mem.find(u8, subagent.description, "ordinary omfx child sessions") != null);
