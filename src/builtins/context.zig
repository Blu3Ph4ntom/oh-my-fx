...
const identity_section =
    ...
    \\
    \\- You are omfx, a local coding CLI assistant with tool access.
...
fn permissionModeContext(permission_mode: types.PermissionMode) []const u8 {
    return switch (permission_mode) {
        ...
        .auto => "Runtime context: permission mode is auto. After configured rules, session grants, and deterministic safe-tool authority, omfx reviews each unresolved sensitive tool call once. An automatic non-allow returns a failed tool result for replanning, and exact repeats reuse that denial. Choose a materially different safe action, or when that result contains approval_request_id call ask_user_question with that exact ID to enter omfx's real permission screen. Do not retry unchanged, invent an ID, or treat generic question or conversation text as approval. Bounded consecutive all-blocked response groups end the turn with ordinary blocker text and never open a permission screen automatically; any successful tool resets that count. Tool admission and exact live revalidation remain authoritative.",
        .yolo => "Runtime context: permission mode is yolo. Omfx permission policy and sandboxing are disabled. Tool lookup, argument validation, execution authority, cancellation, limits, operating-system permissions, and remote authentication remain authoritative.",
    };
}
...
test "runtime context composes exact auto mode with noninteractive blockers and sandbox" {
    ...
    try std.testing.expectEqualStrings(
        "Runtime context: permission mode is auto. After configured rules, session grants, and deterministic safe-tool authority, omfx reviews each unresolved sensitive tool call once. An automatic non-allow returns a failed tool result for replanning, and exact repeats reuse that denial. Choose a materially different safe action, or when that result contains approval_request_id call ask_user_question with that exact ID to enter omfx's real permission screen. Do not retry unchanged, invent an ID, or treat generic question or conversation text as approval. Bounded consecutive all-blocked response groups end the turn with ordinary blocker text and never open a permission screen automatically; any successful tool resets that count. Tool admission and exact live revalidation remain authoritative.",
...
test "gateway_system_prompt: local workspace authority" {
    try expectDefaultPromptContains("You are omfx, a local coding CLI assistant with tool access.");
