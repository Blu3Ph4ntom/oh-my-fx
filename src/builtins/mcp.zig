...
test "adding an MCP server creates the profile directory privately" {
    ...
    try std.testing.expectEqual(io_mod.expectedTestMode(0o700), io_mod.permissionsToMode(dir_stat.permissions) & 0o777);
    const file_stat = try tmp.dir.statFile(io_mod.getIo(), "home/.fx/mcp.json", .{ .follow_symlinks = false });
    try std.testing.expectEqual(io_mod.expectedTestMode(0o600), io_mod.permissionsToMode(file_stat.permissions) & 0o777);
}
