...
fn composeCompactFailureTitleRow(
    ...
    const title_budget: u16 = width -| @as(u16, @intCast(display_width.visibleWidthIgnoringAnsi(row.items)));
    try row_text.appendClipped(alloc, &row, title.items, title_budget);
    const retry_budget: u16 = width -| @as(u16, @intCast(display_width.visibleWidthIgnoringAnsi(row.items)));
...
test "resume menu explains retryable selected session contention" {
    ...
    const cases = [_]struct {
        ...
        .{
            ...
            .message = "This session is open in another omfx. Close it there, then press Enter to retry.",
        },
        .{
