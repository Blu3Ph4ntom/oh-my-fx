...
fn paintActions(
    ...
    switch (projection.cancellationCapability(node.state, node.external_busy)) {
        ...
        .external_owner => try writeLine(alloc, writer, cols, row, limit, "Cancel unavailable: another omfx process owns this child."),
...
test "external owner makes manager cancellation unavailable" {
    ...
    try std.testing.expect(std.mem.find(u8, rendered, "another omfx process owns this child") != null);
