...
test "terminal title writes the label to the caller's output file" {
    ...
    try std.testing.expectEqualStrings("\x1b]2;omfx · release notes\x07", written);
}
...
test "terminal title sanitizes and bounds untrusted labels" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, written, "\x1b]2;omfx · safe]2;owned"));
...
test "resume handoff uses one row only when the full instruction fits" {
    ...
    const single_row = "Continue session with: omfx --resume session-123";
    var exact_buffer: [128]u8 = undefined;
    const exact = try formatResumeHandoff(&exact_buffer, "session-123", single_row.len);
    try std.testing.expectEqualStrings(
        "\x1b[38;5;245mContinue session with: omfx --resume session-123\x1b[0m\n",
        exact,
    );
...
test "resume handoff follows the active muted theme shade" {
    ...
    try std.testing.expectEqualStrings(
        "\x1b[38;5;247mContinue session with: omfx --resume session-123\x1b[0m\n",
        message,
    );
}
...
test "welcomeMessage shows version and help hint" {
    ...
    try std.testing.expect(std.mem.find(u8, message, "omfx") != null);
...
test "welcomeMessage keeps only the app name bright" {
    ...
    const expected = try std.fmt.allocPrint(
        ...
        "{s}omfx{s}{s} v{s} · Run /help for commands" ++ reset_style ++ "\n\n",
